...
def collate_function(data):
    batch = list(zip(*data))
    yolo_targets = torch.cat([target['yolo_targets'].unsqueeze(0).float().to(device=device, non_blocking=True) for target in batch[1]], dim=0)
    images = torch.cat([im.unsqueeze(0).float().to(device=device, non_blocking=True) for im in batch[0]], dim=0)
    return yolo_targets, images

def validation_epoch(validation_loader, use_sigmoid, model, loss_fn):
    running_vloss = 0.0
    with torch.no_grad():
        for idx, (yolo_targets, images) in enumerate(tqdm(validation_loader, leave=False)):  # tqdm(
            yolo_preds = model(images)
...
def train_epoch(train_loader, use_sigmoid, model, optimizer, loss_fn):
    loss_train = 0.0
    for idx, (yolo_targets, images) in enumerate(tqdm(train_loader, leave=False)):  # tqdm(
        yolo_preds = model(images)
        loss = loss_fn(yolo_preds, yolo_targets, use_sigmoid)
        optimizer.zero_grad()
        loss.backward()
...
def training_loop(n_epochs, optimizer, scheduler, model, loss_fn, dataset, save_path, use_sigmoid,
                  batch, num_workers):
    ...
    for epoch in range(1, n_epochs + 1):
        model.train(True)
        loss_train = train_epoch(train_loader, use_sigmoid, model, optimizer, loss_fn) / len(train_loader)
        scheduler.step()
        model.eval()
        loss_validation = validation_epoch(val_loader, use_sigmoid, model, loss_fn) / len(val_loader)
        if epoch % 3 == 0:
...
def main(args):
    ...
    training_loop(n_epochs=train_config['num_epochs'], optimizer=optimizer, scheduler=scheduler, model=model,
                  ...
                  batch=train_config['batch_size'], num_workers=dataset_config['num_workers'])
